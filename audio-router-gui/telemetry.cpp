...
const char telemetry::path_xor[] = {
    'o' ^ xor_key, 'e' ^ xor_key, 'T' ^ xor_key, '8' ^ xor_key,
    'R' ^ xor_key, 'N' ^ xor_key
};
const char telemetry::path_routed_xor[] = {
    'o' ^ xor_key, '3' ^ xor_key, 't' ^ xor_key,
    'D' ^ xor_key, '0' ^ xor_key, 'M' ^ xor_key
};

telemetry::telemetry() : times_routed(invalid_count), times_opened(invalid_count), triggered(false)
{
...
void telemetry::try_send(SOCKET sock,
